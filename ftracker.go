...
func ShowTrainingInfo(action int, trainingType string, duration, weight, height float64, lengthPool, countPool int) string {
	...
	case trainingType == "Бег":
		...
		calories := RunningSpentCalories(action, weight, height) // вызовите здесь необходимую функцию
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	return (walkingCaloriesWeightMultiplier*weight + (math.Sqrt(meanSpeed(action, duration))*kmhInMsec/height)*walkingSpeedHeightMultiplier*weight) * duration * minInH
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	// ваш код здесь
	return (meanSpeed(lengthPool, float64(countPool)) + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight / duration
}
